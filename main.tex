...
Understanding the Method Behind the Machine: interpredability
...
%Y$:\\[0.3cm]
$Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + ... + \beta_n X_n + \epsilon$\\
gene expression is not an independent $X_{mrna}$, which could be correlated with other $X_{body}, X_{stress}, X_{fear}, X_{meaningOfLife}, X_{inflammation}, X_{ros}$ factors.
...
Once holistic feature is available, graph convolutional neural network (GCNN) is mandatory for further survival analysis\cite{Ching2018a}.
