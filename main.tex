...
Xu and his colleagues\cite{Xu2021a} also identified their biomarkers by analysis of differentially-expressed genes (DEGs).
The gene expression was used to select the preliminary biomarkers.
The significant impact on patients' overall survival was evaluated by Kaplan–Meier survival curves with a log-rank test (\textit{P} value $< 0.01$), and univariate Cox regression.
...
They validated the candidate genes by using the Gene Expression Profiling Interactive Analysis tool (GEPIA1/2 database using TCGA datasets, developed by Zefang Tang and his colleagues\cite{Tang2017a}\cite{Tang2019}
...
The workflow, shown in Figure \ref{fig:figure1}, scanned 20,500 human protein-coding genes of the \acrshort{tcga} \acrshort{hnscc} cohort to yield a model with biomarker estimate by gene-expression based survival analysis.
