...
We developed a workflow that includes data retrieving and pre-processing, feature selection, sliding-window cutoff mining, Kaplan-Meier survival analysis, Cox proportional hazard modeling, and biomarker discovery.
Using this workflow on the TCGA HNSCC cohort, we scanned human protein-coding genes. After adjustment with confounders, we found that the clinical tumor stage and the surgical margin involvement are independent risk factors in patient survival. According to the resulting tables with Bonferroni adjusted P-value under the optimal cutoff and the hazard ratio, three biomarker candidates, CAMK2N1, IL19, and FCGBP, are significantly associated with the patients' overall survival. The other independent HNSCC data were also used to validate this discovery.
We suggest the transcriptomic analysis could help for biomarker discovery.}

% Keywords
\keyword{Head and Neck Squamous Cell Carcinoma (HNSCC);
the Cancer Genome Atlas (TCGA); Transcriptomic Analysis; Survival Analysis; Optimal Cutoff; Effect Size; \acrfull{CAMK2N1}; \acrfull{IL19}; \acrfull{FCGBP}; Mindfulness Meditation} 
...
Our approach describes a workflow implemented in the R script, which runs on the Rstudio server.
...
The independent HNSCC datasets were used to validate this strategy.
...
(2) survival analyses of the other HNSCC dataset (GSE65858) using Kaplan-Meier estimate (KM, with FDR correction) and Cox modeling (CoxHR).\\
...
In the study of GSE65858 cohort with median cutoffs, CAMK2N1, IL19, and FCGBP (3 out of those 20 genes discovered in the TCGA cohort), keep ahead of the curve by their FDR-corrected \textit{P} value ($< 0.05$), and Cox's HR ($> 1.8$ or $< 0.65$) (please see Table \ref{tab:table_top3_GSE65858}).
...
%IL19
Conversely, the most associated gene with better survival is \acrfull{IL19}. In Figure \ref{fig:figure4}(c), a Kaplan-Meier curve reveals 215 patients bearing the higher expression of \acrshort{IL19} had 60\% of 5-year OS survival rate (Bonferroni-adjusted \textit{P} $ = 0.002$). The sliding-window cutoff selection generated cumulative \textit{P} value plot in Figure \ref{fig:figure4}(d). This plot reveals a special "S" curve with a acute bending on the middle portion (we will discuss this finding in the Discussions section).
The 161 uncorrected \textit{P} values were estimated by a serial cut from 125 to 286 for grouping the cohort. The smallest \textit{P} value (\num{3.73e-7}), when cut at n = 199 (48.1\% of total cohort 414), has been defined as an optimal \textit{P} value with a cutoff value -0.147 \acrshort{rsem} of \acrshort{rnaseq}.
% FCGBP
...
It shows that the number of genes with unadjusted \textit{P} value $< 0.05$, which is 6284 versus 3118, respectively. After FDR correction, it becomes 967 versus 209, respectively.
...
Then, these preliminary candidates will have an opportunity to be carefully selected by using Bonferroni correction, their effect size (Cox's HR), and the other independent cohorts (GSE65858, GSE2837) to prevent the false discovery.
...
We explain aforementioned situation by an example.
%***NDFIP1  % 給他們一個機會
...
% the effect size
However, it was excluded as a candidate by small effect size (HR = 1.33 in GSE65858 and HR = 1.42 in GSE2837) less than 1.8.
%%%
\end{MyColorPar} % red
...
The genes (e.g. CAMK2N1, and NDFIP1) are also on the list of unfavorable prognostic genes for HNSCC from \acrshort{hpa} (available at https://www.proteinatlas.org/humanproteome/pathology/head+and+neck+cancer, Version: 20.0 updated: 2020-11-19). 
...
It is expressed by the hazard function denoted by h(t). Briefly, the hazard function can be interpreted as the risk of a specific event (e.g., death) at time t. It can be estimated as follow:
...
The Gene Expression Omnibus (GEO) database\cite{Chung2006},, founded by \acrfull{ncbi}, is a public repository supporting MIAME-compliant data submissions of microarray and sequence-based experiments.
...
GSE2837 HNSCC dataset (microarray gene expression by Affymetrix X3P chips U133\_X3P) has 28 participants and RFS information. 
Using PrognoScan (availalbe at http://dna00.bio.kyutech.ac.jp/PrognoScan/)\cite{Mizuno2009a} for the validation of our candidates.
...
Our findings suggested 3 biomarker candidates---CAMK2N1, IL19, and FCGBP, are all heavily associated with the prognosis of \acrshort{os} under optimal cutoff with stringent Bonferroni \textit{P} values and proper effect size (HR). 
...
\supplementary{The following are available online at \linksupplementary{Supplementary_figures.pdf}, 
	...
	Table S2: 10 good guys, % ? don't keep it
