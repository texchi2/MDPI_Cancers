...
$^{5}$ \quad Graduate Institute of Biomedical Informatics, College of Medical Science and Technology\unskip, Taipei Medical University\unskip, Taipei\unskip, Taiwan\\
}
...
After the discovery by our workflow, we validated the suggested 20 biomarkers in the other independent \acrshort{hnscc} cohort (GSE2837).
...
Thus, these 10 out of 20 candidates have been confirmed by a comparative study using the GSE2837 dataset other than the TCGA cohort. Please see Kaplan-Meier plots in Supplementary Figure S1.%\ref{fig:fig_GSE2837}.
...
Our workflow has the advantage to find ai optimal cutpoint of that \acrshort{rna} expression data to maximize coverage during candidate mining.
...
Our 5 candidates (e.g. CAMK2N1, PGK1, SURF4, USP10, and NDFIP1) are also on the list of unfavorable prognostic genes for HNSCC from \acrfull{hpa} (available at https://www.proteinatlas.org/humanproteome/pathology/head+and+neck+cancer, Version: 20.0 updated: 2020-11-19). 
...
PGK1 is responsive in hypoxia status, since it is one of HIF-1\alpha regulated gene revealed in HNSCC\cite{Peng2011}\cite{Shih2017}.
...
\subsubsection{Transcriptomic Representation by Deep Learning}
...
%TCGA Pathology Images => The Cancer Imaging Archive (TCIA) by Deep learning (R/Pytorch/Keras framework)

%數位模擬，能讓臨床醫師和研究人員更快速了解細胞成長後的狀況，而不是花費時間等待細胞的培養。
A deep-learning model, named HE2RNA\cite{Schmauch2020}, could use to encode transcriptomic representation from histological whole slide images of TCGA. 
%The transcriptomic representation of TCGA learned by HE2RNA.
%TCIA is for radiomics deep learning
Even with RNA-Seq data obtained from bulk cells isolated from whole slide images, HE2RNA model has also been able to predict the spatial expression of more than 8,000 genes from the H\&E slide of HNSCC alone.
GDC store pathology Aperio SVS image. An SVS file is a digital slide image file created by an Aperio ScanScope slide scanner or others.
CAMK2N1 and IL19 are candidate markers suggested in our current study.


HE2RNA modeling: Y (RNA-seq) ← X (slide represents gene expression)
...
%x The Stanford Tissue Microarray Database (TMAD; x http://tma.stanford.edu) 
%https://tma.im/cgi-bin/ontologyBrowser.pl?anchor=C34447; there is few HNSCC slides


% TMU joiny IRB
We will introduce TMU HNSCC cohort from our joint biobank.
...
*** TCGA TCIA Pathology Images with deep learning: gene expression on pathological section, or digital tissue microarray, digital TMA

%The Cancer Imaging Archive (TCIA) by Deep learning (R/Pytorch/Keras framework)
...
Trust in Machine Learning *** interpretable or explainable CNN
By saliency mapping: e.x. diabetes retinopathy, model focused on retinal artery
%https://bmcmedgenomics.biomedcentral.com/articles/10.1186/s12920-019-0624-2
%#XAI
Interpretable deep neural network for cancer survival analysis by integrating genomic and clinical data\cite{Hao2019a}
By PyTorch
...
%
HE2RNA was able to spatialize differentially genes specifically expressed by T cells or B cells, even though discriminating between those two types from their morphology alone is notoriously difficult.
HE2RNA could be a useful tool for pathologists and oncologists to consistently quantify immune infiltration or guide treatment decision in the context of immunotherapy\cite{Schmauch2020}.
The practical implications of predicting gene expression level from H\&E slides should be emphasized. For example, it could predict molecular profiling from pathology slides, such as the hormonal status of breast cancer cases.
Recent studies have shown that mRNA and protein expressions may be poorly correlated due to various factors, such as different half-lives and post-transcription machinery.
...
{\\Beyond carcinogenesis - Under the mind-brain-body axis, a stressful environment will trigger emotional reception. The subconscious mind (brain) will release stress hormones and inflammation signals in response to the emotion. The physical body's internal environment (cells) alters epigenetic control in gene regulation and mRNA expression. In a long incubation time, the tissue/cells will be transformed into dysplasia then malignancy with helping from known carcinogens.\\
Cancer care - Holistic care should take care of cancer patients' spiritual, emotional, physical, and socioeconomic needs. Physical care will be carried out by medication therapy or surgery. After establishing a therapeutic relationship, the physicians' spiritual properties (empathy, sympathy, and compassion) will engage their patients and recover their self-compassion to gain resilience against the disease.\\
Suggested healthcare record - Besides physical, pathological, or psychological data, electric healthcare records (EHR) and \acrshort{tcga} should collect "holistic features".}
\label{fig:figure5}
\end{figure}
...
Thus, cortisol is one of the most common (objective) outcomes studied in psychoneuroimmunological (PNI) factors. % others: interleukin, cytokines, lymphocytes (CD4+, CD8+), CD56+, vitals signs, telomere length, telomere activity, HRV
...
The therapeutic relationship is essential towards spiritual care.
...
Epigenetics control says that genetic expression is directly due to the environment of our cells and our perception of the environment\cite{Gustafson2017}. (see Figure \ref{fig:figure5})
% so do the cancer
...
%療癒 
Some studies showed that after 8 hours of meditation, the expression of some inflammatory genes changed\cite{Gustafson2017}.
...
The mind-brain-body axis shows that subconscious fear causes releasing of inflammatory and stress hormones from the brain in terms of cell alteration in DNA methylation, mutation, mRNA expression. Carcinogenesis will occur if the reprogramming is persistent for a longer time. (see Figure \ref{fig:figure5})
...
The possible molecular mechanisms could be related with the mind-brain-body axis\cite{Berens2017}.
...
Our current study summarizes those six candidate biomarkers, clinical T stage, and surgical margin are independent prognosis factors in our \acrshort{hnscc} study.
Thus, the prognosis model with coefficients is established from \acrshort{tcga} \acrshort{hnscc} cohort. The important input $X_1...X_n$ should be patients' features: age, specific six gene expressions, clinical T stage, and surgical margin so far.
When we consider the survival prediction model again,
ground truth Y (e.x. patient's survival):\\[0.5cm]
%Y$:\\[0.3cm]
$Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \beta_3 X_3 + ... + \beta_n X_n + \epsilon$\\[0.5cm]
Gene expression is not an independent $X_{mrna}$. It could be correlated with other\\ $X_{body}, X_{stress}, X_{fear}, X_{meaningOfLife}, X_{inflammation}, X_{ros}$ factors.
...
cortisol is one of the most common objective outcomes studied in psychoneuroimmunological (PNI) factors. Other factors are interleukin, cytokines, lymphocytes (CD4+, CD8+), CD56+, vitals signs, telomere length, telomere activity, HRV.
...
7. Medical relationship: bad experience of dental treatment/ loss confidence to dentist/ anxious to treatment/ side-effects/ litigation experience\\[0.5cm]
...
We continually educate our cancer patients that they should repentantly confess for not taking care of body and spirit in the past, and sincere thanks for our physical body's hard working. Since mindfulness meditation will be helpful in cancer healthcare.
