...
We appreciate the reviewer for the critical comments and thank for your agreement of our workflow in cancer research.

to generalize the purpose of our workflow:
TCGA HNSCC -> TCGA CRC -> TCGA LUAD -> TCGA BRCA -> GEO or other uploaded cancer cohort
how to apply or modify the R scripts to use other datasets
the elements of the workflow:
raw data cleaning
Description:table transformation and merge
data structure (dataset schema): GDAC format
clinical features: dichotomization
R scripts could not be applied to start the analysis of any dataset without re-coding.
...
GEO database could be generalized applied by this workflow, since modification of the R script is necessary.
