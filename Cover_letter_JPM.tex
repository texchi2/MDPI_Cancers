...
The expression data was generated using the platform GPL10558 (Illumina HumanHT-12 v4.0 Expression BeadChip), which targets on more than 30,330 annotated genes (47,000 probes, derived from the \acrshort{ncbi} Reference Sequence, release 38 on November 7, 2009). % (RefSeq) 
...
Total 9416 genes has %\acrshort{fdr}-
unadjusted \textit{P} value less than 0.05.
The 967 genes were kept by criteria of uncorrected \textit{P} value (below 0.05) and \acrfull{hr} (greater than 1.8 or less than 0.6).
%The 10 genes, named as DKK1, CAMK2N1, STC2, PGK1, SURF4, USP10, NDFIP1, FOXA2, STIP1, and DKC1, are significantly associated with the poor prognosis of overall survival (OS). 
...
2) \\
( ) % pvalueTex is essential
There is benefit of using sliding-window cutoff method in survival analysis of TCGA HNSCC cohort.
We have compared the results of cutoff at optimal by sliding-window or just at median of gene expression.
It shows
the gene number with unadjusted \textit{P} value $<0.05$, which is 6284 vs. 3118, respectively.
After FDR-correction, it shows 967 vs. 209, respectively.
...
The sliding-window cutoff method could catch more potential candidates (\textit{P} values far less than $0.001$) for Cox's modeling at next stage in our workflow.
These preliminary candidates must be validated by Bonferroni correction and the other independent cohorts (GSE65858, GSE2837) to prevent the false discovery.
...
We will explain above mentioned by two examples.
%***NDFIP1  % 給他們一個機會
Reviewing the special cases of gene, such as NDFIP1, DKC1, PNMA5, and NPB,
we have noticed that NDFIP1, with a \textit{P} value of 0.05 at 50\% quantile cutoff, could achieve a \textit{P} value of \num{2.62e-6} at 70\% quantile cutoff.
NDFIP1 has the FDR-corrected \textit{P} value as \num[round-precision=3, round-mode=figures,
scientific-notation=true]{0.0001074233} (please see supplementary figure S1). % it is \ref{fig:pvaluePlot_NDFIP1
...
% the effect size
Although it has failed to compete as a candidate by less effect size (HR = 1.33) in GSE65858 cohort, compared with one (HR = 2.027) in TCGA cohort%.
, and the other (HR = 1.42) in GSE2837 cohort.
...
The other example is one of our candidate: IL19.
It has a \textit{P} value plot with acute S-curve bending at the median zone, that lets the FDR-KM \textit{P} value has large difference between median cutoff (\num[round-precision=3, round-mode=figures,
scientific-notation=true]{0.1154315}) and optimal cutoff (\num{6.54e-6}).
The sliding-window selection of cutoffs could boost their statistic significance to pass the correction by both FDR and Bonferroni methods.
%> gene_labeled_golden[!gene_labeled_golden %in% fdr100_KMsurvival$Gene.Symbol[fdr100_KMsurvival$FDR.Pvalue<0.05]]
%[1] "IL19"
Fortunately, IL19 has passed to become a candidate by better effect size (HR = 0.472 in TCGA cohort), and also by the validation with the other independent cohort (HR = 0.630 in GSE65858 cohort%).
...
It might be due to skewed distribution of the expression value in 20500 RNA-Seq  data. % heterogeneity
