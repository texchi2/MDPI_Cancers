...
%{% A SUMMARY
%=A summary modified/answers in cover letter (finish the first page) {
We re-wrote the Introduction with the studies of TCGA database according to the suggestion by reviewer\#2.
%[introduction] ok
%Answer 2-6 minor2 (how to study TCGA)
We introduced an independent HNSCC dataset (GSE65858) to provide more cases and evidence for supporting our biomarker candidates. The Results, Materials and Methods have included this information.
%[results] ok Answer 2-3 (GSE65858 then final 3 candidates)
We modified the Discussions about sample size, sliding window method, and removal of "unclean data" statement according to the suggestion by reviewer\#2.
%[discussions] ok
%Answer 2-3 (sample size, effect size, pvalueTex cutoff of NDFIP1,  and P value)
%purpose of sliding window (ok 提 一下at  2-6 minor3)
%Answer 2-6 minor3 (In line 154 paragraph, what does "unclean data")
And the 'holistic cancer care" was also modified according to the suggestion by reviewer\#1.
%Answer 1-2 (holistic) figure 5
We added the description of FDR correction and Cox modeling in the Materials and Methods according to the suggestion by reviewer\#2.
%[materials and methods] ok
%Answer 2-1 (cutoff finding), replaced a new Figure 5 %(Figure_4_CAMK2N1_IL19_FCGBP.pdf)
%Answer 2-4 (Cox regression)
%Answer 2-3 (GSE65858)
%}
We modified Figure 1 and 2 according to the suggestion by reviewer\#2.
And new Figure 3, 4, 5, Table 1 and 2 was made for the updated results from GSE65858.
We also added new supplementary Figure S1.
...
And all our responses to reviewers' comments are in the following pages.
...
We are updating very much from this major revision.
Thank you very much for the kind helps and the reviewers' critical comments.\\[0.3cm]
...
After morning meditation in a wonderful day of February 2021, the first author (Dr. Chi) has raised this idea then wrote it in discussion section with comprehensive references. Dr. Chi wants to express that "stress should also be a kind of biomarker, which includes into the HNSCC study in the future".
We appreciate the reviewer for the agreement about mindfulness meditation and holistic cancer care. We briefed this section (and Figure 6), then will submit rest of it in the other manuscript (study of electronic health record in survival analysis).
...
Going for holistic cancer care\cite{Mehta2019}\cite{Iftikhar2021}, spiritual and emotional condition is equally essential comparing with physical and social status (see Figure 6).%\ref{fig:figure5}).
...
The patients will trust those who take care of them and have the confidence to increase the capacity to recover from diseases through a mind-brain-body connection manner (see Figure 6).%\ref{fig:figure5}).
"
\end{MyColorPar} %red
\end{MyIndent}
...
Since the sliding-window selection of cutoffs would capture more genes whose \textit{P} values just reach the significant level (\textit{P} = 0.05),
...
we reviewed the results of cutoffs from 50\% to 70\% quantile to find the evidence.
%> View(hazards[hazards$Gene.Symbol %in% c(gene_labeled_silver, gene_labeled_golden), c("Gene.Symbol","FDR.Pvalue")])
A total of 20 preliminary candidates have FDR-corrected \textit{P} values from \num{3.78e-6} to \num{3.03e-4} (please see Table \ref{table:table1} and \ref{table:table3}; also shown in supplementary Table S1, S2).
The top 3 biomarker candidates, CAMK2N1, CALML5, and FCGBP, have FDR-corrected \textit{P} values from \num{1.63e-5} to \num{1.97e-4} (please see Figure \ref{fig:figure4}, 
%which have been grouped by cutoffs between; 
it is also shown in manuscript as updated "Figure 5").
Moreover, the gene NDFIP1, one of the 20 preliminary candidates, has a \textit{P} value (around $0.05$) at 50\% quantile cutoff.
NDFIP1 also has a \textit{P} value of \num{2.62e-6} when cut at 70\% quantile  (please see Figure \ref{fig:pvaluePlot_NDFIP1}, and also in 
...
In those \textit{P}-value plots, we could see that the sliding-window selection of cutoff finding might collect more candidates (\textit{P} values far less than $0.0003$) for further Cox's modeling.
%Some useful biomarkers might be kick-out prematurely in "median cutoff" of survival analysis.
Indeed, it needs to determine its effect size (hazard ratio) then to be validated by the other independent cohort.

%%% supplementary tables
%Supplementary Table \ref{table:table1}: \\
%Overexpression of 10 genes with Cox's hazard ratio $> 1.5$.\\

%Supplementary Table \ref{table:table3}: \\
%Overexpression of 10 genes with Cox's hazard ratio $< 0.6$. \\
...
\begin{tabularx}{\textwidth}{|l|p{6.5cm}|l|l|l|l|l|l|} % *** capital X or p{2cm}
% p{2cm}
\cline{1-8}
\multicolumn{1}{|c|}{\multirow{2}{*}{Gene ID}} & \multicolumn{1}{c|}{\multirow{2}{*}{Gene Description}}     & \multicolumn{2}{c|}{Kaplan-Meier survival}                                                                                   & \multicolumn{2}{c|}{Univariate~}                                                        & \multicolumn{2}{c|}{Multivariate} \\
\cline{3-8}
\multicolumn{1}{|c|}{}                         & \multicolumn{1}{c|}{}                                      & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}FDR \\ \textit{P} value\end{tabular}}               & \multicolumn{1}{c|}{\begin{tabular}[c]{@{}c@{}}Bonferroni \\\textit{P} value\end{tabular}} & \multicolumn{1}{c|}{HR*}                   & 95\% CI                                    & HR*                                        & 95\% CI   \\\cline{1-8}
DKK1                                           & dickkopf WNT signaling pathway inhibitor 1                 & \num{3.8e-6}                               & 0.001                                                                           & 2.266                                      & 1.666-3.082                                & 2.135                                      & 1.559-2.924     \\                          % &                                           \\ 
\cline{1-8}
CAMK2N1                                        & calcium/calmodulin-dependent protein kinase II inhibitor 1 & \num{1.5e-5}                               & 0.002                                                                           & 2.101                                      & 1.572-2.809                                & 2.007                                      & 1.490-2.704  \\                            % &                                           \\ %\checkmark
\cline{1-8}
STC2                                           & stanniocalcin 2                                            & \num{1.5e-5}                               & 0.004                                                                           & 2.147                                      & 1.578-2.921                                & 2.075                                      & 1.515-2.843                             \\%   &                                           \\ 
\cline{1-8}
PGK1                                           & phosphoglycerate kinase 1                                  & \num{2.4e-5}                               & 0.006                                                                           & 2.127                                      & 1.563-2.895                                & 2.046                                      & 1.498-2.795                              \\%    &                                           \\ %\checkmark
\cline{1-8}
SURF4                                          & surfeit 4                                                  & \num{6.2e-5}                               & 0.006                                                                           & 2.055                                      & 1.531-2.757                                & 2.089                                      & 1.543-2.829                               \\%   &                                          \\ %\checkmark  
\cline{1-8}
USP10                                          & ubiquitin specific peptidase 10                            & \num{7.9e-5}                               & 0.012                                                                           & 2.083                                      & 1.532-2.834                                & 2.119                                      & 1.551-2.895                              \\%    &                                          \\ %\checkmark
\cline{1-8}
NDFIP1                                         & Nedd4 family interacting protein 1                         & \num{1.1e-4}                               & 0.017                                                                           & 2.031                                      & 1.502-2.746                                & 2.027                                      & 1.483-2.771                               \\%   &                                           \\ %\checkmark 
\cline{1-8}
FOXA2                                          & forkhead box A2                                            & \num{1.6e-4}                               & 0.018                                                                           & 1.976                                      & 1.479-2.640                                & 1.914                                      & 1.426-2.569                               \\%   &                                         \\ %\checkmark  
\cline{1-8}
STIP1                                          & stress-induced-phosphoprotein 1                            & \num{1.8e-4}                              & 0.029                                                                           & 1.958                                      & 1.463-2.621                                & 1.957                                      & 1.451-2.640                               \\%   &                                           \\ 
\cline{1-8}
DKC1                                           & dyskeratosis congenita 1, dyskerin                         & \num{2.8e-4}                               & 0.042                                                                           & 2.046                                      & 1.490-2.808                                & 1.837                                      & 1.332-2.534                               \\%   &                                          \\ 
\cline{1-8}
...
\multicolumn{8}{|l|}{\begin{tabular}[c]{@{}l@{}}Selection criteria:~~\\~Kaplan-Meier Bonferroni-adjusted \textit{P} $< 0.05 $~\\~Cox's univariate and multivariate $HR > 1.5$~\\~(* Cox's model: \textit{P} $< 0.001$)\\ \end{tabular}}   \\ 
%\cline{1-8}
%\multicolumn{8}{|l|}{Selection criteria:~\\~Kaplan-Meier Bonferroni-adjusted \textit{P} $< 0.05 $~\\~Cox's univariate and multivariate $HR > 1.5$~\\~* Cox's model: \textit{P} $< 0.001$}   \\ 
\cline{1-8}
%\multicolumn{9}{|l!{\color{black}\vrule}}{Remark: \checkmark validated by GSE2837}                i                                                                                                                                                                                                                                                                                                                                             \\
%\hline
\end{tabularx}%
} % resizebox end
...
\begin{table}[ht]
\centering
\caption{The other top 10 genes overexpressed with better prognosis in \acrshort{hnscc} (ranked by adjusted \textit{P} value) }
\arrayrulecolor[rgb]{0.255,0.255,0.255}
\resizebox{\linewidth}{!}{%
...
ZNF557                                         & zinc finger protein 557                                & \multicolumn{1}{l!{\color{black}\vrule}}{\num{4.7e-6}} & \multicolumn{1}{l!{\color{black}\vrule}}{0.001}                                                      & \multicolumn{1}{l!{\color{black}\vrule}}{0.465} & \multicolumn{1}{l!{\color{black}\vrule}}{0.348-0.619} & \multicolumn{1}{l!{\color{black}\vrule}}{0.499} & \multicolumn{1}{l!{\color{black}\vrule}}{0.372-0.669}                                              \\ 
\cline{1-2}\arrayrulecolor{black}\cline{3-8}%\arrayrulecolor[rgb]{0.255,0.255,0.255}\cline{9-9}
ZNF266                                         & zinc finger protein 266                                & \num{5.2e-6}                                           & 0.001                                                                                                & 0.474                                           & 0.355-0.632                                           & 0.453                                           & 0.338-0.607                                                                                       \\ 
\hline
IL19                                           & interleukin 19                                         & \num{6.5e-6}               & 0.002                                                                                                & 0.472                                           & 0.351-0.635                                           & 0.459                                           & 0.340-0.619                                          \\%   &                                             \\ %\checkmark
\hline
MYO1H                                          & myosin 1H                                              & \num{1.4e-5}               & 0.003                                                                                                & 0.468                                           & 0.347-0.632                                           & 0.467                                           & 0.344-0.634                                          \\%   &                                              \\ 
\hline
FCGBP                                          & Fc fragment of IgG binding protein                     & \num{4.8e-5}               & 0.008                                                                                                & 0.484                                           & 0.359-0.653                                           & 0.496                                           & 0.366-0.674                                        \\%     &                                             \\ %\checkmark
\hline
%LOC148709                                      & LncRNA LOC148709                                       & \textcolor[rgb]{0,0,0.471}{\num{1.5e-6}}                                           & 0.010                                                                                                & 0.499                                           & 0.374-0.666                                           & 0.485                                           & 0.361-0.652                                           &                                              \\ 
%\hline
EVPLL                                          & envoplakin-like protein                                & \num{7.5e-5}                                           & 0.013                                                                                                & 0.490                                           & 0.363-0.661                                           & 0.494                                           & 0.364-0.672                                            \\% &                                              \\ 
\hline
PNMA5                                          & paraneoplastic antigen like 5                          & \num{3.0e-4}                                           & 0.017                                                                                                & 0.499                                           & 0.371-0.671                                           & 0.481                                           & 0.357-0.650                                            \\% &                                              \\ 
\hline
IQCN            & IQ motif containing N               & \num{1.5e-4}                                           & 0.020                                                                                                & 0.500                                           & 0.371-0.673                                           & 0.483                                           & 0.356-0.654                  \\%                           &                                              \\ %\checkmark
\hline
NPB                                            & neuropeptide B                                         & \num{2.6e-4}                                           & 0.027                                                                                                & 0.460                                           & 0.328-0.646                                           & 0.457                                           & 0.324-0.646                                            \\% &                                              \\ %\checkmark
\hline
CALML5 &
\acrlong{CALML5} &
   \num{2.0e-4} & %\num{6.54e-6} & %3.7e-7} &
   ...
   0.379-0.686 &
   0.493 &
   0.364-0.667  \\%  &  \\  % \checkmark \multicolumn{2}{l|} {
\hline
\multicolumn{8}{|l|}{\begin{tabular}[c]{@{}l@{}}Selection criteria:~\\~Kaplan-Meier Bonferroni-adjusted \textit{P} value \textless{} 0.05~\\~Cox's univariate and multivariate HR \textless{} 0.6\\
%\hline
%\multicolumn{9}{|l!{\color{black}\vrule}}{
(* Cox's model: \textit{P} $< 0.001$) \\
%}                                                                              
%lncRNA: Long non-coding RNA\\%Remark: \checkmark Validated by GSE2837
\end{tabular}}                                                                   \\ \hline
\end{tabular}
}
\arrayrulecolor{black}
...
%four overexpressed genes (symbol as IL19, FCGBP, IQCN - former symbol as KIAA1683, and NPB) are good guy.
%}
in Figure \ref{fig:hazards3}, have FDR-adjusted \textit{P} values ($< \num{3.0e-4}$) in Kaplan-Meier estimate, and survival impact on HR ($> 1.8$ or $< 0.6$) in Cox's model.
%
% [2021/06/13-17]
...
However, the other 17 genes (discovered in TCGA) are replaced by genes such as DUSP6, MSMB, and RBM11 (please see Figure \ref{fig:hazards534}).
Thus, there is consensus between the TCGA and GSE65858 cohorts that CAMK2N1, CALML5, and FCGBP are significant candidates for the biomarker.
...
% results ok
the paragraphs in "Results" marked in \textcolor{red}{Red} from page 5 (line 147) to page 6 (line 169) , and
% ok
...
In the next step, a stringent Bonferroni P-value correction was used to yield 20 genes (please see Figure 2(c), (d)).
%, under the stringent criteria 
%em dash ---
...
We tried to integrate multiple genes in survival analysis, using a group of co-regulated genes that are preferable to be taken into consideration in a gene signature.
...
Branders and his colleagues\cite{Branders2014} suggested that regularization of regression model should be applied on analysis of the high-dimensional data, e.g., twenty thousands of features (genes) and only hundreds of sample size.
...
In such a case, over-fitted model will only perfectly match every single case, and has higher variability to predict what it never see.
...
% multinomial
%RLassoCox\\
We utilized the RLassoCox\cite{Simon2011}\cite{Liu2021} (RLasso-Cox model, implemented in a bioconductor package, available at https://github.com/weiliu123/\\RLassoCox) to find the gene signature.
%for training both Cox proportional hazard regression and logistic regression models to identify a group of genes that contribute to survival outcome of HNSCC patient. 
...
The RLassoCox makes use of both Cox and LASSO logistic regression for gene-expression based survival analysis of HNSCC.
...
LASSO calculate the (non-zero) coefficients of features to hit the minimum of cross-validation error.
The results shows that the model-selected features (62 genes) distribute within the region of small effect size (HR from 0.6 to 1.7) on our previous volcano plots (please see Figure \ref{fig:Bayes62}; comparing with Figure \ref{fig:hazards3} and Figure \ref{fig:hazards534}), if see them individually.
...
We further limited gene selection (13 out of 62) by the most regularized model under cross-validation error within 1 standard deviation of the minimum.
According to their \textit{P} values (please see Table \ref{tab:signature4}), 
we found a gene signature, HK1, EZR, RRAGA, and DUSP16, which will has survival impact on TCGA HNSCC cohort (please see Figure \ref{fig:signature4}).
DUSP16 has negative coefficient in the LASSO regression model, compatible with HR less than 1.0 on the volcano plot of TCGA HNSCC.
...
\begin{table}[ht]
\centering
\caption{The most LASSO regularized logistic model (under cross-validation error within 1 standard deviation of the minimum). Genes with positive coefficient will increase the patient's risk, while negative reduce it.\\
...
%# "EZR" has \textit{P} value $>0.05$
  \caption{The LASSO-Cox regression model predict a set of gene signature. (a) Overall survival in TCGA HNSCC for the gene signature including HK1, EZR, RRAGA, and DUSP16 (Kaplan-Meier \textit{P} value = \num{1.31e-6}); (b) HK1, RRAGA, and DUSP16 have small effect size---hazard ratio within 0.6-1.7, if calculated individually. And EZR has Kaplan-Meier \textit{P} value $>0.05$. (\textcolor{red}{Red}-squared genes are the biomarker candidates found by sliding-window cutoff survival analysis)}
    \label{fig:signature4}
\end{figure}
%(listed on the right side)


In conclusion, we might build a logistic regression model with LASSO $L_{1}$-regularization to maximize the use of continuous gene expression values (without cutoff).
...
% 不要誇大說明 謙虛道歉
We apologize for exaggerating the difficulties in our work.
% ok
The paragraphs marked in \textcolor{red}{Red} on page 11 (line 286) to page 12 (line 314) are the part of the manuscript that has been revised. 
We want to share the experience for this HNSCC data pre-processes in detail to help further survival analysis in other TCGA cohorts.
We added a paragraph of "Technical Consideration".
Also, it is shown here in the following two paragraphs.\\
We are improving very much from this major revision.
Thanks for the reviewers' kind help again.

\begin{MyIndent}
\begin{MyColorPar}{red}
%%%
"
3.3 Technical Consideration\\[0.2cm]
...
According to the reviewer's comment, the Discussions was modified accordingly. The paragraphs marked in \textcolor{red}{Red} from page 11 (line 257) to page 13 (line ?) are the part of the manuscript that has been revised. Also, it is shown here in the following paragraph.
\\[0.3cm]
