...
The p-values are affected by the sample size, which should be described in the main text. % writing; also affecting the over-fitting during a model training
...
In the study of GSE65858 cohort, CAMK2N1, IL19, and FCGBP, keep ahead of the curve by FDR-corrected \textit{P} value ($< 0.05$), and Cox'HR ($>1.8$ or $<0.65$) (please see Table \ref{tab:table_top3_GSE65858} listed below,
...
***...the \textit{P} value is affected by the sample size. % TCGA has the most HNSCC sample size, if combine several GSE65858 and with TCGA HNSCC 
An increase of the sample size will decrease overfitting.
...
We appreciate the reviewer for the critical comments.
 %=> includes gene signature that are related to a grouping structures with similar biological functions tend to have similar expressions. 
A group of co-regulated genes that are preferable to be taken into consideration in a gene signature.
In the data science literature, feature selection has been usually applied for a group of genes which are contributing as independent variables for building model such as logistic regression. %The dependent variables are the survival data.
...
The feature selection (subsets of gene) could be done by the $L_{1}$-regularized path algorithm in LASSO regression. That is suggested by Branders and his colleagues\cite{Branders2014} applying on those high-dimensional data i.e., twenty thousands of features (genes) and only hundreds of sample size.
LASSO is suitable at picking up a small signal (gene signature) through lots of noise (all protein-coding genes).
...
Cross-validation is perhaps the simplest and most widely used method for select one of models at specific \lambda. % k-fold Cross-validation
As the coefficients of logistic regression are reduced to zero by LASSO, the useless features associated with those coefficients will be omitted.
The important features that contribute much to the model will be selected.
Cross-validation error is calculated to find the optimal value of the regularization rate (i.e., lambda, \lambda). \lambda could control the size of a small amount of bias.
% over-fitting causes problem
During fitting of logistic model, it is necessary to add that small amount of bias for getting a significant drop in the variance to prevent it's over-fitting.  
%(over-fitting is called high-variance)
In case with over-fitting, this trained model will only perfect match every individual cases, while has higher variability to predict what it never see.
Over-fitting could be controlled well by using both LASSO $L_{1}$-regularization and cross validation.

%RLassoCox\\
We utilized the RLassoCox\cite{Simon2011}\cite{Liu2021} (RLasso-Cox model, implemented in a bioconductor package, available at https://\\github.com/weiliu123/RLassoCox) for training the Cox proportional hazard regression and multinomial logistic regression models in gene expression analysis to identify a group of genes that contribute to the survival outcome of HNSCC patient. %and gene interaction networks (The KEGG network\cite{Simon2011}).
...
The RLassoCox make use of both tasks for gene-expression based survival analysis.
...
#code inside: fit.lasso <- glmnet(x.train, y.train, family="gaussian", alpha=1) # L1-regularization, LASSO
%Rplot_GSE65858_HNSCC_CoxHR_CAMK2N1_Bayes62_FDRKM.pdf
% Rplot_TCGA_HNSCC_CoxHR_CAMK2N1_Bayes62_FDRKM.pdf
the selected features (62 genes) are selected by their (non-zero) coefficients. 
%cv.mod.candidate 62 genes 
However, they are not performed well either in GSE65858 HNSCC or TCGA HNSCC (please see Figure \ref{fig:Bayes62}).\\
...
The right vertical line shows us the most regularized model with the cross-validation error within 1 standard deviation of the minimum.
% in lambda 1se
%cv.mod$glmnetRes$lambda.1se
the most regularized model with cross-validation error within 1 standard deviation of the minimum.
It selects 14 genes,
according to the \textit{P} value, 4 gene signature: HK1, EZR, RRAGA, and DUSP16.



% lasso 就是要找出 coefficient of regression non-zero 的基因: selected features
Finally, we found a gene signature, HK1, EZR, RRAGA, and DUSP16 to predict their survival impact on TCGA HNSCC cohort (please see Figure \ref{fig:signature4}).

However, # 4 gene signature: #### 
# "EZR" has \textit{P} value $>0.05$

Conclusion, we might build a Lasso regression model with L1 regularization to maximize the use of continuous gene expression values (without a cutoff).
The gene signature integrating multiple genes is also a good approach for biomarker discovery.

\begin{figure}
%    \centering
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=8cm]{rplot_KMplot_HK1.pdf}
  ...
  \caption{(a) OS in TCGA HNSCC for a gene signature including HK1, EZR, RRAGA, and DUSP16; (b) Those genes have not significant survival impact if calculated individually (EZR is not shown on volcano plot due to it's \textit{P} value $>0.05$)}
