...
This strategy only identifies more weak “biomarkers,” and it also captures more false positives. % => NOT weak 其實 very significant
Adjustment of multiple test p-values should be applied here, which is missing in the current manuscript.
% PPV positive prediction value => prevalence-sensitive (COVID-19 盛行率的觀念一致) ROC from sensitivity and specificity
...
% 1) ***We modified the workflow as ...(本來就有計算 FDR and excluding >= 0.05)
% 2) and FDR (done)
...
%% *** 準備一張 20 candidates: accumulated \textit{P}-value plots 來說明，其實這些 P values 不論 50% cut or 70% 幾乎都 far less than 0.05
...
% strategy should changing; run GCP Rstudio 框列更多的 candidates for GEO validation
...
As I mentioned above, the authors should look for more data to verify these biomarkers. % GEO
...
% using  receiver operating characteristic (ROC) analysis and more GEO datasets
...
x Prognoscan (GSE2837) (n=30) 不行，太少 n=30, and RFS only: validation rate 14/100 而且 10% cutoff
...
% National Center for Biotechnology Information Reference Sequence (NCBI) RefSeq Release 38 (November 7, 2009) 


GEOquery\cite{Sean2007}: SOFT file (or MINiML)
the NCBI GEO repositories
...
by TMA IHC of HNSCC (provided by Prof. Michael Hsiao)
or CPTAC (digital TMA, not HPA)i

or more GEO datasets (*GEO2837, GSE40774, ***GSE65858, *GSE117973, GSE148944, and GSE41613/xGSE42743)
* GSE2837 https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE2837

*** https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE65858 GSE65858
https://onlinelibrary.wiley.com/doi/full/10.1002/ijc.29649
OS event and RFS event and survival time
n=270\cite{Wichmann2015}
Each array on the HumanHT-12 v4 Expression BeadChip targets more than 31,000 annotated genes with more than 47,000 probes derived from the \acrfull{ncbi} Reference Sequence (RefSeq) Release 38 (November 7, 2009) and other sources.
...
*x* \cite{Zhao2018} used GSE41613 (with survival data of 97 HNSCC cohort) for DEGs.
GEO2R: GSE41613 and GSE42743 可是*** has only 131 genes of the Affymetrix array data % https://www.ncbi.nlm.nih.gov/geo/geo2r/?acc=GSE41613
from 97 HPV-negative OSCC patients %on the 131 gene 
to identify a model for prediction of OSCC-specific survival\cite{Lohavanichbutr2013}.

x 7. Integration of molecular gene signatures with clinicopathological variables improves staging and outcome prediction of oral cancer.
(Submitter supplied) The aim of this study is to identify and validate clinically implementable gene signatures for outcome prediction of patients with oral squamous cell carcinoma.
Organism:	Homo sapiens
Type:		Expression profiling by array
Platform: GPL6480 99 Samples
FTP download: GEO (TXT) ftp://ftp.ncbi.nlm.nih.gov/geo/series/GSE84nnn/GSE84846/
Series		Accession: GSE84846	ID: 200084846
only alive or death (no survival time) => Similarly preprocessed VUmc (GSE84846) and UMCU (GSE30788) microarray datasets (https://www.oncotarget.com/article/19576/text/)
\cite{Mes2017}

%as following: % search by StarGEO ===========
StarGEO http://stargeo.org/ searching: HNSCC + survival (n>100]
* GSE2837 https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE2837


x https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE40774
n=134; Agilent-026652 Whole Human Genome Microarray 4x44K v2 (Probe Name version)
supplementary data:
https://clincancerres.aacrjournals.org/content/21/4/870.figures-only#fig-data-additional-files
n=134 (no survival)
# Version info: R 3.2.3, Biobase 2.30.0, GEOquery 2.40.0, limma getGEO("GSE40774", GSEMatrix =TRUE, cex=1.5)library("maptools") 




x https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE148944
   784 probes (gene）
n=40; 110 FFPE samples were collected from a retrospective cohort of 67 OTSCC patients and 10 non-cancerous tongue samples. Targets including CD4, CD8, FOXP3, PD-L1 and PD-1 were analyzed by immunohistochemistry. The Nanostring PanCancer Immune Profiling Panel was used for gene expression profiling. Data were externally validated in the TCGA head and neck (HNSCC), melanoma and lung squamous cell carcinoma (LSCC) cohorts.
GPL28285 NanoString nCounter PanCancer Immune Profiling Panel


https://onlinelibrary.wiley.com/doi/full/10.1002/cam4.3106
.................
