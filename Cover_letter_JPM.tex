...
The larger the sample the more likely a difference to be noticed (i.e. \textit{P} value $< 0.05$), due to the decrease of the standard error\cite{Sullivan2012}.
...
Conversely, an effect size can be large, but fail to get statistical significance if the sample size is small. 
%In this scenario, the \textit{P} value tells the effect does not exist.

The \textit{P} value could be also impacted by error, which includes:
\begin{outline}
...
There is a benefit for using sliding-window cutoff method (avoiding a skewed cutoff) in Kaplan-Meier analysis of TCGA HNSCC cohort at the beginning.
We have compared the results of cutoff at optimal \textit{P} value by sliding-window or just at median of gene expression.
...
That is because of proper selected cutoff improving the statistical significance.
...
% the effect size
Then it has been excluded as a candidate by less effect size (HR = 1.33) in GSE65858 cohort, compared with one (HR = 2.027) in TCGA cohort%.
...
It has a \textit{P} value plot with acute S-curve bending at the median zone, that lets the FDR-adjusted \textit{P} value has large difference between 50\% quantile cutoff (FDR 0.115)
%(\num[round-precision=3, round-mode=figures, scientific-notation=true]{0.1154315}) 
and optimal 48\% quantile cutoff (FDR \num{6.54e-6}).
This optimal cutoff method could boost it's statistic significance to pass the correction by both FDR and Bonferroni methods.
%> gene_labeled_golden[!gene_labeled_golden %in% fdr100_KMsurvival$Gene.Symbol[fdr100_KMsurvival$FDR.Pvalue<0.05]]
%[1] "IL19"
Fortunately, IL19 has competed to become a candidate by its better effect size (HR = 0.472 in TCGA cohort), and also by the validation with the other independent cohort (HR = 0.630 in GSE65858 cohort).
