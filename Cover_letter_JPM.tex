...
[after cut-off finding]
Adjustment of multiple test p-values should be applied here, which is missing in the current manuscript.

***=> Bonferroni correction should be used in final decision.
%位置要換一下：
The false discovery rate
645 (FDR) (< 0.05) could be used to pick up the optimal P value to ensure the control for
646 type I error of the Kaplan-Meier survival test during the cutoff finding procedure.
...
We change new Title, abstract, change tone
...
= 詳細描述 univariable and multivariable tests work
In a multivariable test, which variables were used? 
= 列出來，並且強調 surgical margin
...
We appreciate the reviewer for the critical comments and thanks for the agreement of our workflow in cancer research.
[Minor comments]
The background should be revised extensively. 
x For example, the description of the linear model should be removed. X1...Xn should be removed. 
***More introduction to how other researchers used TCGA to find expression biomarkers should be presented. 
Detailed introduction of TCGA-related tools should be moved to Methods.

In line 154 paragraph, what does unclean data mean, missing values or zero expression? The three challenges are not “challenges” at all. The authors exaggerated the difficulties here. 不要誇大說明 
In Figure 1, the diagram should have direction. 
加上 direction of view 

The file names are not interesting. RNAseq (20500 genes) should be gene expression values (20500 genes)???

In Figure 2, unlabeled scales on the x-axis???
除去 unlabeled stick 即可立刻改進
\begin{MyColorPar}{red}
