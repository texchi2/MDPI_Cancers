...
As skewed cutoff that split 425 vs 75 between the two groups, it will also get statistical significance by increasing effect size intentionally.
...
when \textit{P} value $<0.05$, the gene number: 9416 , 534 %差別很大，張大網
run code: if median cut for TCGA KM Cox (by pvalueTex or *** RLassoCox Bayes code) 9416 -> ???
...
(  )2-3 put in supplementary figure Rplot_pvaluePlot_NDFIP1.pdf
%列入 workflow in materials and methods;

***NDFIP1
a special case of NDFIP1 gene; % 給他們一個機會
(ok) Answer 2-1 %加入 discussion
( )  Answer 2-3 supplementary  %圖就好 figure Rplot_pvaluePlot_NDFIP1.pdf
...
scientific-notation=true]{0.0001074233}.
However, it fails as the candidate by inadequate effect size (HR = ??).
% (ok) Answer 2-1 加入 discussion
